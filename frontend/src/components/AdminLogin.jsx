...
export default function AdminLogin() {
  ...
  const handleLogin = () => {
    const valid = adminUsers.some(
      (u) => u.username === username && u.password === password
    );
    if (valid) {
      // navigate to dashboard and pass username in state
      // persist admin session in cookie (1 day)
      try {
        Cookies.set("adminUser", username, { expires: 1 });
      } catch (e) {
        // ignore cookie errors
      }
      navigate("/admin-dash", { state: { username } });
    } else {
  ...
  useEffect(() => {
    // if admin is already stored in localStorage, skip login
    try {
      const stored = Cookies.get("adminUser");
      if (stored) {
        navigate("/admin-dash", { state: { username: stored } });
      }
    } catch (e) {
      // ignore
    }
        ...
        <main className="mt-6">
          <section className="bg-gray-800/40 border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl backdrop-blur-lg">
            ...
            <p className="text-sm text-gray-300 mb-6">
              Use your admin credentials to access the system tools.
            </p>
            ...
            <form onSubmit={handleSubmit} className="grid gap-4">
              ...
              <div className="mt-6 flex items-center gap-3">
                <button
                  type="submit"
                  className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:scale-105 transition-transform rounded-lg shadow-lg"
                >
                  Sign In
                </button>
                <button
