...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://192.168.1.5:8000/user/forgot-password", {
        email,
      });
  ...
  const handleVerifyOtp = async () => {
    setLoading(true);
    try {
      const response = await axios.post("http://192.168.1.5:8000/user/verify-forgot-password", {
        otpGenerated,
        otp,
        email
      });
